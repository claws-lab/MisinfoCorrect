...
def _get_inputs_from_text(text, tokenizer, if_input_src_only=False):
	...
	if if_input_src_only:
		# ==== condition 1 ====: but, see so many split tokens in the text generation
		# srcs = src_seeker_post + ' <SPLIT> ' + ' <END>'
		# ==== condition 2 ====: like Dialog-GPT setup
		srcs = src_seeker_post + ' <|endoftext|> '
	else:
...
def _make_features(id_, inputs, src_seeker_post, src_response_post, tokenizer, max_len):
	...
	for ids in inputs:
		...
		len_ += (len(ids) + 1)
		sents.append(ids)
	#  : it seems sents is just a concated
	if len(sents) >= 1:		
...
def _make_feature(id_, sents, src_seeker_post, src_response_post, eos):
	#  : change from list of list to a single list
	input_ids = [i for s in sents for i in s+[eos]][:-1]

	weights = []
	token_type_ids = []  # this becomes round ids

	#  : in the input index list: we have split in the middle
	# like: source <SPLIT> reply
	...
	#  : here, we have three important ids:
	# 1) input_ids: sequential; 2) position_ids: provide the position information
	# 3) token_type: for the related split/EOS tagging
...
def main(args):
	...
	attrs = []
	if args.if_input_src_only:
		print(f"we only keep source text in the input")
		attrs.append("if_input_src_only")
	if args.reverse:
	...
	with open(args.corpus, "r", encoding="utf-8") as reader, \
		...
		for line in tqdm(reader, total=_get_file_len(args.corpus)):
			...
			try:
				...
				#  : inputs will have the whole seeker post+response post after tokenization and encoding
				inputs, src_seeker_post, src_response_post  = _get_inputs_from_text(line, toker, if_input_src_only=args.if_input_src_only)
				if args.reverse:
					inputs = list(reversed(inputs))
				if args.two_turn:
					inputs = inputs[:2]
				#  : when running the code, it is one line with only one seeker post and response post
				# print(f"inputs is: {inputs}\n{type(inputs)}")
				# inputs: list of list: [[22087, 13, 10478]]
				features = _make_features(n_example, inputs, src_seeker_post, src_response_post,
										  toker, args.max_seq_len)

				# print('features:', features) #  : no need to see it:
				# features: [<gpt2_training.train_utils.InputFeatures object at 0x7f83af68ee20>]
...
if __name__ == '__main__':
	...
	parser.add_argument('--two_turn', action='store_true',
						help='take only the first 2 turns: for toy examples? guessed by  ')
	parser.add_argument('--if_input_src_only', action='store_true', default=False,
						help='by  : input with src only')
