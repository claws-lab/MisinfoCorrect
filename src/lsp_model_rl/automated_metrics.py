...
# ext: relevance reward - coherence reward
def relevance(seeker_post, predicted):
	from numpy import dot
	from numpy.linalg import norm
	import numpy as np
	def encode(post):

		inputs = bert_tokenizer.encode_plus(post, return_tensors="pt", add_special_tokens=True)
		outputs = bert_model(**inputs)

		# [0] hidden state of all tokens in the sentence, [1] the final last only 1 hidden state output
		last_hidden_states = outputs[1] 
		return last_hidden_states

	# Get embeddings
	seeker_post_embeddings = encode(seeker_post)
	predicted_response_embeddings = encode(predicted)
